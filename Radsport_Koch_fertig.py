...
class RadsportApp(ctk.CTk):
    ...
    def generate_html_invoice(self, b_id, k_name, a_name, b_menge, b_datum, a_preis):
    ...
    def show_bestellungen_liste(self):
        ...
        def lade_liste():
            ...
            if conn:
                try:
                    cur = conn.cursor()
                    # WICHTIG: a.preis wurde in den SELECT aufgenommen, um die Rechnung zu berechnen!
                    cur.execute("""
                    ...
                    for b in cur.fetchall():
                        ...
                        # NEU Button: Rechnung anzeigen
                        btn_rechnung = ctk.CTkButton(row, text="📄 Rechnung", width=80, fg_color="#f39c12",
                                                     text_color="black", hover_color="#d68910",
                                                     command=lambda bid=b_id, kn=k_name, an=a_name, bm=b_menge,
                                                                    bd=b_datum, ap=a_preis: self.generate_html_invoice(
                                                         bid, kn, an, bm, bd, ap))
